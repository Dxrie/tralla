<div class="d-flex flex-column flex-shrink-0 p-3 bg-light h-100" style="width: 280px;">
    <div class="d-flex justify-content-center align-items-center flex-column mb-4">
        <div class="position-relative" style="width: 56px; height: 56px;">
            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR1Ge5ZNd1jtWQIUJVaZhczOC8H1JNSlg443g&s" 
                 class="w-100 h-100 object-fit-cover rounded-circle border"
                 alt="{{ $user->name ?? 'User' }}">
        </div>
        <span class="mt-2 px-2 fw-medium">{{ $user->name ?? 'User' }}</span>
        <p class="px-2 text-muted small"><span class="text-capitalize">{{ $user->role ?? 'Employee' }}</span> of PT. Tralla</p>
    </div>
    ...
    <ul class="nav nav-pills flex-column mb-auto">
        ...
        <!-- Absensi Dropdown -->
        <li class="nav-item">
            <a class="nav-link link-dark d-flex align-items-center justify-content-between" 
               data-bs-toggle="collapse" 
               href="#absensiCollapse" 
               role="button" 
               aria-expanded="false" 
    ...
    <ul class="nav nav-pills">
        <li class="nav-item">
            <form method="POST" action="{{ route('logout') }}" class="w-100">
                @csrf
                <button type="submit" class="nav-link link-dark d-flex align-items-center w-100 border-0 bg-transparent">
